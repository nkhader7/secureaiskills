---
name: audit-asvs-compliance
description: Audits an application, API, design, or codebase against OWASP Application Security Verification Standard 5.0 requirements. Use when reviewing ASVS compliance, mapping implementation evidence to ASVS controls, producing an ASVS gap assessment, or checking application security requirements by ASVS level.
triggers:
...
1. Load `references/rules.yaml` to get the ASVS 5.0 requirement catalog.
...
3. Determine the assessment scope:
...
4. Evaluate each applicable rule using `match_strategy: compliance_review`.
...
5. For every non-pass result, capture the ASVS ID, level, chapter, section, status, evidence, gap, impact, and recommended remediation.
6. Aggregate results by ASVS chapter, verification level, and status.
